...
└── native/MenuBar/         — Swift menu-bar .app (SPEC_MENUBAR_UI_2026-07-30): spawns `meet start --headless`, controls via SIGINT/SIGUSR1/SIGUSR2/SIGWINCH; ad-hoc signed, Dock-less, optional launch-at-login
    ...
    ├── SettingsWindowController.swift — programmatic settings window (no nib): curated toggles over config.json; Live Model + Final Model popups list `~/.meet/models/*.bin` (alias label, current selected, custom-path fallback), Save writes liveModelPath/finalModelPath — picked up by the pipeline per chunk
